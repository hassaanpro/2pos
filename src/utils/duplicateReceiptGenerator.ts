...
/**
 * Generate a duplicate receipt with clear marking
 */
export const generateDuplicateReceipt = async (
  ...
  try {
    ...
    // Generate receipt HTML with DUPLICATE watermark
    const receiptHTML = `
      <!DOCTYPE html>
      <html>
      <head>
        <title>DUPLICATE Receipt #${receiptData.receipt_number}</title>
        <style>
          body {
      ...
      <body>
        ...
        <div class="duplicate-notice">
          DUPLICATE RECEIPT - NOT VALID FOR RETURNS
        </div>
        ...
        <div class="receipt-info">
          ...
          <div><span class="bold">Original Date:</span> ${new Date(receiptData.sale_date).toLocaleString('en-PK')}</div>
        ...
        <div class="reprint-info">
          <div><span class="bold">REPRINT INFORMATION</span></div>
          <div>Reprint #: ${reprintInfo.reprint_count}</div>
          <div>Reprinted By: ${reprintInfo.reprinted_by}</div>
          <div>Reason: ${reprintInfo.reason}</div>
          <div>Auth Code: ${reprintInfo.auth_code}</div>
          <div>Reprint Date: ${new Date().toLocaleString('en-PK')}</div>
        </div>
        
        <div class="footer">
          <div>${receiptFooter}</div>
          <div>This is a duplicate receipt and is not valid for returns or exchanges.</div>
          <div>Original receipt was issued on ${new Date(receiptData.sale_date).toLocaleDateString('en-PK')}</div>
        </div>
      </body>
      </html>
    `;
    ...
    setTimeout(() => {
      receiptWindow.print();
      console.log('Duplicate receipt printed:', receiptData.receipt_number);
    }, 500);
